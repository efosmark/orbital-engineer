...
DASH = [3.0, 3.0]
PRIMARY_BODY_RADIUS = 15
SECONDARY_BODY_RADIUS = 7
...
def draw_true_anomaly(cr, fx, fy, x_true, y_true, nu):
    ...
    # True anomaly arc (around periapsis)
    cr.set_source_rgba(*COLOR_TRUE_ANOMALY, 0.3)
...
class EccentricAnomalyDiagram(Gtk.DrawingArea):
    ...
    def on_draw(self, area, cr:cairo.Context, width, height):
        ...
        # Focus (central mass)
        draw_body(cr, fx, fy, PRIMARY_BODY_RADIUS, color=COLOR_CENTRAL_BODY)

        # Orbiting body
        draw_body(cr, x_true, y_true, SECONDARY_BODY_RADIUS, color=COLOR_ORBIT)
        
        # Ghost orbit
        draw_body(cr, x_circ, y_circ, SECONDARY_BODY_RADIUS, color=(*COLOR_ECCENTRIC_ANOMALY, 0.4))
        
        # Apoapsis
        draw_body(cr, x_apo, y_apo, SECONDARY_BODY_RADIUS, color=COLOR_GHOST)
        text = "apoapsis"
        te = cr.text_extents(text)
        cr.move_to(x_apo - SECONDARY_BODY_RADIUS - te.width - 5, y_apo + (te.height/4.0))
        cr.show_text(text)

        # Periapsis
        draw_body(cr, x_peri, y_peri, SECONDARY_BODY_RADIUS, color=COLOR_GHOST)
        text = "periapsis"
        te = cr.text_extents(text)
        cr.move_to(x_peri + SECONDARY_BODY_RADIUS + 5, y_peri + (te.height/4.0))
        cr.show_text(text)
