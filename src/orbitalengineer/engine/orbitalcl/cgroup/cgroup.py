...
class CGroupPipeline(CLPipelineStep):
    ...
    def find_contacting_bodies(self, num_pairs: int, pairs: cl.Buffer, flags: cl.Buffer, position: cl.Buffer, radius: cl.Buffer, time_of_interaction: cl.Buffer):        
        cl.enqueue_copy(self.queue, self.num_contacts_by_lane_cl, np.zeros(self.N * self.N, dtype=np.uint32)).wait()
        cl.enqueue_copy(self.queue, self.contacts_by_lane_cl, np.zeros(self.N * self.N, dtype=np.uint32)).wait()
        
        self.tr.add("find_contacting_bodies",
            self._find_contacting_bodies(
                self.queue,
                (int(num_pairs), ),   # global work size
                #(self.Lx, ),           # local work size
                
                #(num_pairs, ),  # global work size
                (self.Lx,   ),  # local work size
    ...
    def cgroup_assign(self, cgroups: cl.Buffer):
        ...
        while updated and num_iterations < 1000:
            ...
            if num_iterations >= self.N:
                raise Exception("TOO MANY ITERATIONS")
    
    def __call__(self, num_pairs: int, pairs: cl.Buffer, flags: cl.Buffer, position: cl.Buffer, radius: cl.Buffer, time_of_interaction: cl.Buffer, cgroups_buffer: cl.Buffer):
        self.find_contacting_bodies(num_pairs, pairs, flags, position, radius, time_of_interaction)        
        self._print_collisions_per_body()
